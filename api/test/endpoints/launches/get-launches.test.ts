...
jest
  .spyOn(axios, "post")
  .mockImplementation(async (url: string): Promise<AxiosResponse<any, any>> => {
    const mockedResult =
      MOCKED_UPSTREAM_RESPONSE_DATA[
        url as keyof typeof MOCKED_UPSTREAM_RESPONSE_DATA
      ];
    if (mockedResult) {
      return {
        data: mockedResult,
...
test("populated twice", async () => {
  ...
  jest
    .spyOn(populateOneFieldModule, "populateOneField")
