...
jest.spyOn(getLaunchesModule, "getLaunches").mockImplementation((_:any, res:Response)=>res.send({message:"OK"}) as any)

const app = {
  get: jest.fn((_:string, func:(req:Request, res:Response)=>any)=>{
    func({query:{}} as any, {status:jest.fn().mockReturnThis(), send:jest.fn()} as any);
  }),
...
test("we have enough endpoints", ()=>{
  ...
  expect(app.get).toHaveBeenCalledWith("/v1/launches/", getLaunchesModule.getLaunches)
