 ...
 */
export async function populateOneField(
  launches: SpaceXQueryResult<SpaceXLaunchPopulated>,
  key: "launchpad" | "rocket",
  url: string
) {
  type NameIdPair = { name: string; id: ObjectIdString };

  const subDocIdList = launches.docs.map(
    (doc) => doc[key as keyof SpaceXLaunchPopulated]
  );
  // deduplicate IDs
  ...
  const subDocNameDict = Object.fromEntries(
    subDocNameResult.data.docs.map((pair) => [pair.id, pair.name])
  );
