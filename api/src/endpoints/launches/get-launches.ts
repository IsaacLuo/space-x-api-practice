...
import { URL_LAUNCHES_QUERY, URL_LAUNCHPADS_QUERY, URL_ROCKETS_QUERY } from "../../upstream-urls";
...
export async function getLaunches(req:Request, res:Response):Promise<void> {
  ...
  const selectedKeys = ["rocket", "success", "details", "launchpad", "name", "date_utc", "id"];
  let response;
  try {
    response = await axios.post<SpaceXQueryResult<SpaceXLaunch>>(URL_LAUNCHES_QUERY, {options: {select: selectedKeys, page:page}});
  } catch {
  ...
  try {
    ...
    console.error(error);
    res.status(500).send({message: "Unable to generate launches information"});
  }
}